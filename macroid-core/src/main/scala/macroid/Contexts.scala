...
case class ActivityContextWrapper(original: WeakReference[Activity],
                                  application: Context)
    extends ContextWrapper { type C = Activity }

case class ServiceContextWrapper(original: WeakReference[Service],
                                 application: Context)
    extends ContextWrapper { type C = Service }

case class ApplicationContextWrapper(original: WeakReference[Application],
                                     application: Context)
    extends ContextWrapper { type C = Application }

case class GenericContextWrapper(original: WeakReference[Context],
                                 application: Context)
    extends ContextWrapper { type C = Context }

object ContextWrapper {
  def apply(activity: Activity): ActivityContextWrapper =
    ActivityContextWrapper(WeakReference(activity),
  ...
  def apply[F](fragment: F)(
      implicit fragmentImpl: Fragment[F]): ActivityContextWrapper =
...
case class FragmentManagerContext[-F, M](manager: M)(
    implicit val fragmentApi: FragmentApi[F, M, _]) {
  def get = manager
}

trait Contexts[X] { self: X ⇒
  implicit def activityContextWrapper(
      implicit activity: X <:< Activity): ActivityContextWrapper =
    ContextWrapper(activity(self))

  implicit def fragmentContextWrapper(
      implicit fragment: Fragment[X]): ActivityContextWrapper =
    ContextWrapper(self)(fragment)

  implicit def serviceContextWrapper(
      implicit service: X <:< Service): ServiceContextWrapper =
    ContextWrapper(service(self))

  implicit def viewContextWrapper(implicit view: X <:< View): ContextWrapper =
    ContextWrapper(view(self))

  implicit def activityManagerContext[M, F, A >: X](
      implicit fragmentApi: FragmentApi[F, M, A]) =
    FragmentManagerContext[F, M](fragmentApi.activityManager(self))

  implicit def fragmentManagerContext[M, F >: X, A](
      implicit fragmentApi: FragmentApi[F, M, A]) =
