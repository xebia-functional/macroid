...
@implicitNotFound(
  "Don't know how to excerpt from ${W} with ${E}. Try importing an instance of CanExcerpt[${W}, ${E}, ...].") /** A typeclass for 'excerptable' relation */
...
object CanExcerpt {
  ...
  implicit def `Ui is excerptable`[W, E, R](
      implicit canExcerpt: CanExcerpt[W, E, R]): CanExcerpt[Ui[W], E, R] =
    new CanExcerpt[Ui[W], E, R] {
      def excerpt(ui: Ui[W], e: E) = ui flatMap { w ⇒
        canExcerpt.excerpt(w, e)
      }
    }
}
...
private[macroid] trait Excerpting {
  ...
  implicit class ExcerptingOps[W](w: W) {
    ...
    /** Excerpt a value */
    def ~>[E, R](e: E)(implicit canExcerpt: CanExcerpt[W, E, R]): Ui[R] =
      canExcerpt.excerpt(w, e)
  }
}
