...
private[macroid] trait DensityQueries extends MediaQueryEssentials {
  def ldpi(implicit ctx: ContextWrapper) =
    MediaQuery(displayMetrics.densityDpi == DisplayMetrics.DENSITY_LOW)
  def mdpi(implicit ctx: ContextWrapper) =
    MediaQuery(displayMetrics.densityDpi == DisplayMetrics.DENSITY_MEDIUM)
  def hdpi(implicit ctx: ContextWrapper) =
    MediaQuery(displayMetrics.densityDpi == DisplayMetrics.DENSITY_HIGH)
  def xhdpi(implicit ctx: ContextWrapper) =
    MediaQuery(displayMetrics.densityDpi == DisplayMetrics.DENSITY_XHIGH)
}

private[macroid] trait OrientationQueries {
  def portrait(implicit ctx: ContextWrapper) =
    MediaQuery(
      ctx.application.getResources.getConfiguration.orientation == Configuration.ORIENTATION_PORTRAIT)
  def landscape(implicit ctx: ContextWrapper) =
    MediaQuery(
      ctx.application.getResources.getConfiguration.orientation == Configuration.ORIENTATION_LANDSCAPE)
}
...
private[macroid] trait SizeQueries extends MediaQueryEssentials {
  ...
  /** Width is at least `v` */
  def minWidth(v: Int)(implicit ctx: ContextWrapper) =
    MediaQuery(displayMetrics.widthPixels >= v)

  /** Width is at least `v`(alias for `minWidth`) */
  def widerThan(v: Int)(implicit ctx: ContextWrapper) = minWidth(v)

  /** Width is at most `v` */
  def maxWidth(v: Int)(implicit ctx: ContextWrapper) =
    MediaQuery(displayMetrics.widthPixels <= v)
  ...
  def minHeight(v: Int)(implicit ctx: ContextWrapper) =
    MediaQuery(displayMetrics.heightPixels >= v)

  /** Height is at least `v` (alias for `minHeight`) */
  def higherThan(v: Int)(implicit ctx: ContextWrapper) = minHeight(v)

  /** Height is at most `v` */
  def maxHeight(v: Int)(implicit ctx: ContextWrapper) =
    MediaQuery(displayMetrics.heightPixels <= v)
