...
private[macroid] trait BasicSnails {
  ...
  def delay(millis: Long) =
    Snail[View](x ⇒ Future(())(snailSchedulerEc(millis)))

  /** A snail that waits for a given future to finish */
  def wait(f: Future[Any])(implicit ec: ExecutionContext) =
    Snail[View](x ⇒ AfterFuture(f))
}

private[macroid] trait ProgressSnails
    extends BasicSnails
    with VisibilityTweaks {

  /** Show this progress bar with indeterminate progress and hide it once `future` is done */
  def waitProgress(future: Future[Any])(
      implicit ec: ExecutionContext): Snail[ProgressBar] =
  ...
  def waitProgress(futures: List[Future[Any]])(
      implicit ec: ExecutionContext): Snail[ProgressBar] =
    Tweak[ProgressBar] { x ⇒
      ...
      x.setProgress(0)
      futures.foreach(f ⇒
        AfterFuture(f).foreachUi(_ ⇒ Ui(x.incrementProgressBy(1))))
...
private[macroid] trait AnimationSnails extends VisibilityTweaks {
  ...
  /** Fade in this view */
  def fadeIn(millis: Long) =
    show ++ anim(new AlphaAnimation(0, 1), duration = millis)

  /** Fade out this view */
  def fadeOut(millis: Long) =
    anim(new AlphaAnimation(1, 0), duration = millis) + hide
}
