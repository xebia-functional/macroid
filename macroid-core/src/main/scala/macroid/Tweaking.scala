...
@implicitNotFound(
  "Don't know how to tweak ${W} with ${T}. Try importing an instance of CanTweak[${W}, ${T}, ...].") /** A typeclass for 'tweakable' relation */
...
object CanTweak {
  ...
  implicit def `Widget is tweakable with Effector`[W <: View, F[+ _], T, R](
      implicit effector: Effector[F],
      canTweak: CanTweak[W, T, R]): CanTweak[W, F[T], W] =
    new CanTweak[W, F[T], W] {
      def tweak(w: W, f: F[T]) = Ui {
        effector.foreach(f)(t ⇒ canTweak.tweak(w, t)); w
      }
    }

  implicit def `Effector is tweakable`[W, F[+ _], T, R](
      implicit effector: Effector[F],
      canTweak: CanTweak[W, T, R]): CanTweak[F[W], T, F[W]] =
    new CanTweak[F[W], T, F[W]] {
      def tweak(f: F[W], t: T) = Ui {
        effector.foreach(f)(w ⇒ canTweak.tweak(w, t)); f
      }
    }

  implicit def `Ui is tweakable`[W, T, R](
      implicit canTweak: CanTweak[W, T, R]): CanTweak[Ui[W], T, W] =
    new CanTweak[Ui[W], T, W] {
      def tweak(ui: Ui[W], t: T) = ui flatMap { w ⇒
        canTweak.tweak(w, t).withResult(w)
      }
    }
}
...
private[macroid] trait Tweaking {
  ...
  implicit class TweakingOps[W](w: W) {
    ...
    /** Apply a tweak */
    def <~[T, R](t: T)(implicit canTweak: CanTweak[W, T, R]): Ui[R] =
      canTweak.tweak(w, t)
  }
}
