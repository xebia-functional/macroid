...
object ResourcesExtras {
  ...
  private def resGetResource[A](resource: String, resourceType: String)(
      f: (ContextWrapper, Int) ⇒ A)(implicit c: ContextWrapper): Option[A] = {
    val resourceId = c.bestAvailable.getResources
      .getIdentifier(resource, resourceType, c.bestAvailable.getPackageName)
    resourceId match {
      case 0 ⇒ None
  ...
  def resGetColor(resourceId: Int)(implicit c: ContextWrapper): Int =
    ContextCompat.getColor(c.bestAvailable, resourceId)

  def resGetColor(resource: String)(implicit c: ContextWrapper): Option[Int] =
    resGetResource(resource, "color")((c, resourceId) ⇒
      ContextCompat.getColor(c.bestAvailable, resourceId))
  ...
  def resGetInteger(resourceId: Int)(implicit c: ContextWrapper): Int =
  ...
  def resGetQuantityString(resourceId: Int, quantity: Int, formatArgs: AnyRef*)(
    ...
    c.bestAvailable.getResources
      .getQuantityString(resourceId, quantity, formatArgs: _*)
