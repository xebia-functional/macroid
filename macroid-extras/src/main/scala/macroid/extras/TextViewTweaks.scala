...
object TextViewTweaks {
  ...
  val tvBold: Tweak[W] =
    Tweak[W](x ⇒ x.setTypeface(x.getTypeface, Typeface.BOLD))

  val tvItalic: Tweak[W] =
    Tweak[W](x ⇒ x.setTypeface(x.getTypeface, Typeface.ITALIC))
  ...
  def tvEllipsize(truncateAt: TruncateAt): Tweak[W] =
    Tweak[W](_.setEllipsize(truncateAt))

  def tvAllCaps(allCaps: Boolean = true): Tweak[W] =
    Tweak[W](_.setAllCaps(allCaps))
  ...
  def tvDrawablePadding(padding: Int): Tweak[W] =
    Tweak[W](_.setCompoundDrawablePadding(padding))
