...
package object viewable {
  implicit class ViewableOps[A](data: A) {
    ...
    def view[W <: View](implicit ctx: ContextWrapper,
                        viewable: Viewable[A, W]) =
      viewable.view(data)
  }
  ...
  implicit class PagerAdapterOps[A](data: Seq[A]) {
    ...
    def pagerAdapter[W <: View](implicit ctx: ContextWrapper,
                                viewable: Viewable[A, W]) =
      viewable.pagerAdapter(data)

    /** Create a tweak to set the `PagerAdapter` with this sequence */
    def pagerAdapterTweak[W <: View](implicit ctx: ContextWrapper,
                                     viewable: Viewable[A, W]) =
  ...
  implicit class ListAdapterOps[A](data: Seq[A]) {
    ...
    def listAdapter[W <: View](implicit ctx: ContextWrapper,
                               listable: Listable[A, W]) =
      listable.listAdapter(data)

    /** Create a tweak to set the `ListAdapter` with this sequence */
    def listAdapterTweak[W <: View](implicit ctx: ContextWrapper,
                                    listable: Listable[A, W]) =
